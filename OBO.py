...
class OBO(object):
    ...
    def get_term_relations(self, v):
        ...
        return [ (u, data['r']) for u, data in self.g[v].items() ]
    ...
    def get_ontology_extended_id_mapping(self, terms=None):
        ...
        for key, values in self.precalculated_descendants.items():
            for value in values:
    ...
    def get_nested_ontology_mapping(self, from_ontology_prefix,
        ...
        for v, data in self.g.nodes(data=True):
            ...
            while len(relations) > 0:
                for child, values in relations.items(): # Currently tracing only the last child
        ...
        #for key, value in hsdl_to_hge.items():
