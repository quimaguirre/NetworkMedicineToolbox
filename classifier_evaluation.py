...
def create_R_script(file_name, absolute_dir, title=None, only=None, show_spread=False, vertical_average=False, append=False):
    ...
    f.write("v<-read.table(\"{}predictions.dat\")\n".format(absolute_dir)) 
    f.write("l<-read.table(\"{}labels.dat\")\n".format(absolute_dir))
    ...
    if only == "auc":
        ...
        if append:
            f.write("sink(\"{}{}_auc.txt\", append=TRUE, split=TRUE)\n".format(absolute_dir, title))
        else:
            f.write("sink(\"{}auc.txt\", append=TRUE, split=TRUE)\n".format(absolute_dir))
        ...
        if vertical_average:
            if show_spread:
                ...
                f.write("plot(perfPPV, lwd=2, col=2, ylab=\"Percentage\", main=\"{}\", avg=\"vertical\", plotCI.col=2, spread.estimate=\"stddev\", show.spread.at=seq(0,d,by=d/6))\n".format(plot_title))
            else:
                f.write("plot(perfPPV, lwd=2, col=2, ylab=\"Percentage\", main=\"{}\", avg=\"vertical\")\n".format(plot_title))
        else:
            f.write("plot(perfPPV, lwd=2, col=2, ylab=\"Percentage\", main=\"{}\", xlim=c(0,0.4), ylim=c(0,1))\n".format(plot_title))
        if vertical_average:
            if show_spread:
    ...
    if show_spread:
        f.write("plot(perfROC, lwd=2, col=2, xlab=\"False Positive Rate\", ylab=\"True Positive Rate\", main=\"{}\", avg=\"vertical\", plotCI.col=2, spread.estimate=\"stddev\", show.spread.at=seq(0,1,by=0.20))\n".format(plot_title))
    else:
        f.write("plot(perfROC, lwd=2, col=2, xlab=\"False Positive Rate\", ylab=\"True Positive Rate\", main=\"{}\", avg=\"vertical\")\n".format(plot_title))
    f.write("legend(\"bottomright\", c(\"(Avg. over xval folds)\"), lty=c(1), col=c(2))\n") 
    f.write("dev.off()\n")
    # Cutoff (PPV - Sens)
    f.write("perfPPV<-performance(pred, \"ppv\")\n")
    f.write("perfSens<-performance(pred, \"sens\")\n")
    f.write("png(\"{}cutoff.png\")\n".format(absolute_dir))
    if title is None:
        plot_title = "Precision vs Sensitivity"
    if show_spread:
        f.write("d<-average(perfPPV@x.values)\n")
    ...
    f.write("png(\"{}auc.png\")\n".format(absolute_dir))
    f.write("perfAUC<-performance(pred, \"auc\")\n")
    f.write("e=c(); n=c(); x=0; for ( i in perfAUC@y.values ) { x<-x+1;  e[x] <- i; n[x]<-x }; barplot(e, names=n, ylim=c(0,1),ylab= \"AUC\",xlab=\"Fold\", main=\"{}\")\n".format(plot_title))
    ...
    f.write("sink()\n")
    f.close()
    #os.system("R CMD BATCH {}".format("*.R"))
    return
...
def create_ROCR_files(list_node_scores_and_labels, file_predictions, file_labels):
    ...
    for i, node_scores_and_labels in enumerate(zip(*list_node_scores_and_labels)):
        if i == 0:
            for j in range(len(node_scores_and_labels)):
                f_pred.write("\tFold" + str(j+1))  
                f_lab.write("\tFold" + str(j+1))  
            f_pred.write("\n")  
            f_lab.write("\n")  
        f_pred.write("{}".format(i+1))
        f_lab.write("{}".format(i+1))
...
def get_validation_node_scores_and_labels(file_result, file_seed_test_scores, file_node_scores, n_random_negative_folds = None, n_negatives = None, default_score = 0, replicable = 123, candidates_file = None, previous_negative_sample_size=None):
    ...
    non_seeds = set([ node for node, score in initial_to_score.items() if score==default_score ])
...
def calculate_performance_metric_counts_using_random_negatives(node_to_score, setNodeTest, non_seeds, score_threshold, n_random_negative_folds = None, replicable=123):
    ...
    for id, score in node_to_score.items(): # if candidates based - for each candidate
    ...
    if n_random_negative_folds == 0:
        for id, score in node_to_score.items():
            if id in non_seeds:
                if score >= score_threshold:
                    nFP += 1
                else:
