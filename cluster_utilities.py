...
def run_proximity_on_cluster(parameters, source_to_geneids, target_to_geneids, run_mode='array job', convert_names=True, md5_conversion=False, n_start = 0, n_end = 640000):
    ...
    if run_mode == "run cluster":
        ...
        while i < n_end+1:
            ...
            #        continue
            score_command = "-p {}/input/ -i {} -j {}".format(parameters.get("data_dir"), i, i + increment)
            os.system("sbatch -x node30 run_proximity.sh {}".format(score_command))
            i += increment
        return
    for source, geneids_source in source_to_geneids.items(): 
        ...
        if md5_conversion:
            ...
            if md5 in md5_to_sources:
                ...
                md5_to_sources.setdefault(md5, set()).add(source)
            source = md5
        for target, geneids_target in target_to_geneids.items():
            #print(target, len(geneids_target))
            if convert_names:
                target = text_utilities.convert_to_R_string(target)
            out_file = parameters.get("output_dir") + "/{}_{}.out".format(source, target) 
            if cluster_dir is not None:
                out_file = out_file.replace(parameters.get("base_dir"), cluster_dir) 

            if os.path.exists(out_file):
                    continue
            score_command = ' -x {} -m {} -n {} -e "{}" -o "{}" -s "{}" -t "{}"'.format(seed, min_bin_size, n_random, network_file, out_file, ",".join(geneids_source), ",".join(geneids_target))
            ...
            if run_mode == "array job":
                ...
                print("qsub -cwd -S /bin/bash -o out -e err -v PATH=$PATH -v PYTHONPATH=$PYTHONPATH -q {} -N {}_{} -b y {}".format(qname, source[:3], target[:3], score_command))
            elif run_mode == "run local":
                print("{}".format(score_command))
                ...
                raise ValueError("Unknown run_mode: {}".format(run_mode))
    n = 0
    for md5, sources in md5_to_sources.items():
        if len(sources) > 1:
            n += len(sources) - 1
            print(len(sources))
...
def run_guild_on_cluster(parameters, target_to_geneids, run_mode='array job', method = 's'):
    ...
    nodes = network.nodes()
    for target, geneids in target_to_geneids.items():
        #print(target, len(geneids_target))
        target = text_utilities.convert_to_R_string(target)
        target_to_score = dict((gene, 1.0) for gene in geneids)
        out_file = parameters.get("output_dir") + "/{}.n{}".format(target, method)
        if os.path.exists(out_file):
            continue
...
def output_proximity_results(parameters, sources, targets, out_file, source_to_targets=None):
    ...
    for source in sources: 
        for target in targets:
            ...
            if source_to_targets is not None:
                f.write("{}\t{}\t{}\t{}\n".format(source, target, target in source_to_targets[source], z))
            else:
                f.write("{}\t{}\t{}\n".format(source, target, z))
    f.close()
    return
...
def get_proximity_values(parameters, source_to_geneids, target_to_geneids, dump_file=None, convert_names=True, md5_conversion=False):
    ...
    for source, geneids_source in source_to_geneids.items(): 
        ...
        for target in target_to_geneids:
            ...
            out_file = parameters.get("output_dir") + "/{}_{}.out".format(source_mod, target_mod) 
...
def get_guild_values(parameters, targets, source_to_genes, method='s', dump_file=None):
    ...
    for target in targets:
        target_mod = text_utilities.convert_to_R_string(target)
        out_file = parameters.get("output_dir") + "/{}.n{}".format(target_mod, method) 
        if not os.path.exists(out_file):
            print("File not found:", out_file)
        ...
        m = numpy.mean(values)
        s = numpy.std(values)
        for source, genes in source_to_genes.items(): 
