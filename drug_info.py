...
def get_drug_info(parameters, parser=None):
    ...
    drug_to_geneids = dict((drug, geneids) for drug, geneids in drug_to_geneids.items() if drug in selected_drugs)
    ...
    for drug in drugs:
        ...
        if name not in drug_to_db_id:
            if use_text_matching == True:
                found = False
                for drug, db_name in parser.drug_to_name.items():
...
def get_drug_smiles_by_target(parameters, parser=None):
    ...
    for drug, smiles in parser.drug_to_smiles.items():
...
def get_chemical_similarity_based_target_predictions(parameters, smiles_list, cutoff=0.9, method="fishers"):
    ...
    if method == "fishers":
        ...
        for smiles in all_smiles:
            for smiles_query, smiles_to_value in smiles_to_smiles_similarity.items():
                try:
        ...
        for smiles in smiles_list:
            for geneid, smiles_strings in geneid_to_smiles_strings.items():
                values = []
...
def get_drug_side_effect_subset(parameters, drug_to_side_effects):
    ...
    pairs = [ (drug, side_effect) for drug, side_effects in drug_to_side_effects.items() for side_effect in side_effects ]
...
def get_side_effect_target_symbols(parameters, source = "sider", output_file=None):
    ...
    for side_effect, targets in side_effect_to_targets.items(): 
        values = [] 
...
def get_side_effect_targets_fishers(drug_to_geneids, drug_to_side_effects, cutoff=0.2, correct_pvalues=True): # min_n_drug=5, 
    ...
    for drug, side_effects in drug_to_side_effects.items():
    ...
    n_less_than_five = 0
    for side_effect, drugs_se in side_effect_to_drugs.items():
        #if len(drugs_se) < min_n_drug: 
        #    n_less_than_five += 1
        #    continue
        values = []
        for target, drugs_target in target_to_drugs.items():
...
def get_offsides(parameters):
    ...
    for cid, values in cid_to_values.items():
        cid = cid[3:]
        if cid.startswith("1"):
            cid = "{}".format(abs(int(cid)) - 100000000)
        else:
            cid = "{}".format(abs(int(cid)))
...
def get_nugent(parameters):
    ...
    drug_to_side_effects = {}
    not_in_db = set()
    for name, side_effect_to_count in name_to_side_effect_counts.items():
        ...
        if name in name_to_drug:
            ...
            continue
        count_total = 0.0
        for side_effect, count in side_effect_to_count.items():
            count_total += count
        for side_effect, count in side_effect_to_count.items():
