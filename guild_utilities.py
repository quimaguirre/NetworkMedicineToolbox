...
def main():
    ...
    run_scoring(scoring_folder, random_networks_folder, executable_path, scoring_type="netcombo")
    ...
    node_scores_file = os.path.join(scoring_folder, "node_scores.sif")
    edge_scores_file = os.path.join(scoring_folder, "edge_scores_netshort.sif")
...
def prepare_scoring(network_file, seed_file, scoring_folder=".", random_networks_folder=".", non_seed_score=0.01, seed_score=1.0, edge_score=1.0, n_sample=100, delim=" ", name=None):
    """
        ...
        random_networks_folder: path to directory where the random networks (for netzcore) will be created and stored
    ...
    edge_score_file = os.path.join(scoring_folder, "edge_scores.sif") #network_file.split("/")[-1] + ".converted"
    if fileExist(edge_score_file):
    ...
    node_score_file = os.path.join(scoring_folder, name + "node_scores.sif") #seed_file.split("/")[-1] + ".converted"
    seed_score_file = os.path.join(scoring_folder, name + "seed_scores.sif")
    ...
    bg_node_file = os.path.join(scoring_folder, name + "node_scores_background.sif") #seed_file.split("/")[-1] + ".converted"
    ...
    nd_edge_file = os.path.join(scoring_folder, name + "edge_scores_netshort.sif") #network_file.split("/")[-1] + ".converted_for_netshort"
    ...
    if os.path.exists(sampling_prefix+"{}".format(n_sample)):
        ...
        for i in range(1,n_sample+1):
            ...
            network_utilities.output_network_in_sif(g_sampled, sampling_prefix+"{}".format(i))
    return
...
def create_edge_score_file(edge_score_file, edges, edge_to_data, edge_score=1.0, delim=" "):
    ...
    for edge in edges:
        ...
        f.write("{}{}{}{}{}\n".format(edge[0], delim, score, delim, edge[1]))
    f.close()
...
def create_background_score_file(bg_node_file, bg_seed_file, nodes, seeds, seed_to_data, non_seed_score=0.01, delim=" "):
    ...
    if seed_to_data is not None: seed_scores = list(seed_to_data.values())
    for node in nodes:
...
def run_scoring(scoring_folder, random_networks_folder, executable_path, scoring_type="netscore", parameters={"n_iteration":2, "n_repetition":3, "n_sample":100, "sampling_prefix":"./sampled_graph.", "nd_edge_file":"edge_scores_netshort.sif"}, qname=None, calculate_pvalue=True, xval=None, name=None):
    """
        ...
        qname: name of the queue to be used in the cluster, one of None | "short" | "normal"  | "long" | "bigmem"
    ...
    def score(scoring_type, qname, node_file, edge_file, output_file, parameters):
        if scoring_type == "netscore": 
            score_command = executable_path + ' -s s -n "{}" -e "{}" -o "{}" -r {} -i {} &> "{}"'.format(node_file, edge_file, output_file, parameters["n_repetition"], parameters["n_iteration"], output_file+'.log')
        elif scoring_type == "netzcore": 
            score_command = executable_path + ' -s z -n "{}" -e "{}" -o "{}" -i {} -x {} -d "{}" &> "{}"'.format(node_file, edge_file, output_file, parameters["n_iteration"], parameters["n_sample"], parameters["sampling_prefix"], output_file+'.log')
        elif scoring_type == "netshort": 
            score_command = executable_path + ' -s d -n "{}" -e "{}" -o "{}" &> "{}"'.format(node_file, parameters["nd_edge_file"], output_file, output_file+'.log')
        else:
    ...
    if scoring_type == "netcombo":
        ...
        for scoring in ("netscore", "netzcore", "netshort"):
            ...
            output_score_file = '{}.{}'.format(output_file, scoring)
            if not fileExist(output_score_file):
                score(scoring, qname, node_file, edge_file, output_score_file, parameters)
            if xval is not None:
                for i in range(1, xval+1):
                    output_score_fold_file = '{}.{}.{}'.format(output_file, i, scoring)
                    if not fileExist(output_score_fold_file):
                        score(scoring, qname, node_file+".{}".format(i, edge_file, output_score_fold_file, parameters))
            if calculate_pvalue:
                output_background_score_file = '{}.{}'.format(bg_output_file, scoring)
                if not fileExist(output_background_score_file):
                    score(scoring, qname, bg_node_file, edge_file, output_background_score_file, parameters)
                output_pvalue_file(output_score_file, output_background_score_file, seed_file, bg_seed_file)
                if xval is not None:
                    for i in range(1, xval+1):
                        output_score_fold_file = '{}.{}.{}'.format(output_file, i, scoring)
                        output_background_score_fold_file = '{}.{}.{}'.format(bg_output_file, i, scoring)
                        if not fileExist(output_background_score_fold_file):
                            score(scoring, qname, bg_node_file+".{}".format(i, edge_file, output_background_score_fold_file, parameters))
                        output_pvalue_file(output_score_fold_file, output_background_score_fold_file, None, bg_seed_file)

        # Netcombo
        score_combined([output_file+".netscore", output_file+".netzcore", output_file+".netshort"], output_file+".netcombo")
        if xval is not None:
            for i in range(1, xval+1):
                score_combined(scoring, qname, node_file+".{}".format(i), edge_file, output_file+".{}".format(i, parameters))
        if calculate_pvalue:
            ...
            if xval is not None:
                for i in range(1, xval+1):
                    score_combined(scoring, qname, bg_node_file+".{}".format(i), edge_file, bg_output_file+".{}".format(i), parameters)
                    output_pvalue_file(output_file+".netcombo"+".{}".format(i), bg_output_file+".netcombo"+".{}".format(i), None, bg_seed_file)
    else:
        #if "sampling_prefix" not in parameters:
        #    parameters["sampling_prefix"] = sampling_prefix
        output_score_file = '{}.{}'.format(output_file, scoring_type)
        score(scoring_type, qname, node_file, edge_file, output_score_file, parameters)
        if xval is not None:
            for i in range(1, xval+1):
                output_score_fold_file = '{}.{}.{}'.format(output_file, i, scoring_type)
                score(scoring_type, qname, node_file+".{}".format(i), edge_file, output_score_fold_file, parameters)
        if calculate_pvalue:
            output_background_score_file = '{}.{}'.format(bg_output_file, scoring_type)
            score(scoring_type, qname, bg_node_file, edge_file, output_background_score_file, parameters)
            output_pvalue_file(output_score_file, output_background_score_file, seed_file, bg_seed_file)
            if xval is not None:
                for i in range(1, xval+1):
                    output_score_fold_file = '{}.{}.{}'.format(output_file, i, scoring_type)
                    output_background_score_fold_file = '{}.{}.{}'.format(bg_output_file, i, scoring_type)
                    score(scoring_type, qname, bg_node_file+".{}".format(i), edge_file, output_background_score_fold_file, parameters)
                    output_pvalue_file(output_score_fold_file, output_background_score_fold_file, None, bg_seed_file)
    return
...
def score_combined(scores_file_list, output_scores_file, combination_type="standard", reverse_ranking=False):
    ...
    for scores_file in scores_file_list:
        ...
        if combination_type == "standard":
            mean, sigma = stat_utilities.calc_mean_and_sigma(list(node_to_score_inner.values()))
...
def output_pvalue_file(score_file, background_file, seed_file=None, background_seed_file=None, delim="\t"):
    ...
    values = [ (v, k) for k,v in node_to_significance.items() ]
    values.sort()
    i = 0
...
def get_significance_among_node_scores(node_to_score, background_to_score, n_fold=10000, n_sample = 1000): 
    ...
    values = list(background_to_score.values())
...
def get_node_to_description(node_mapping_file, network_file):
    ...
    node_to_desc = {}
    for node, vals in id_mapping.items():
        selected_val = vals[0]
        for val in vals:
...
def get_top_nodes(pvalue_file, selection_type="pvalue", seed_file=None, cutoff=None, exclude_seeds=False, delim=" "):
    ...
    if selection_type == "sigma":
        ...
        values = []
        for node, vals in node_to_values.items():
            try:
                # p-val file
