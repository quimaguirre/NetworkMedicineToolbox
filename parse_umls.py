...
def get_mesh_id_mapping(desc_file, rel_file, only_diseases = True, dump_file = None): 
    ...
    for concept_id, values in umls.concept_id_to_values.items():
...
def get_drug_info(desc_file, rel_file):
    ...
    for drug, diseases in drug_to_diseases.items():
            print(drug, diseases)
    return
...
def old_get_disease_specific_drugs(umls, drug_to_name, phenotypes):
    ...
    drug_id_to_exp = {} 
    for drug_id, keyword in drug_to_name.items():
        try:
            for l in "[{}]":
    ...
    for drug, diseases in drug_to_diseases.items():
        ...
        #print(drug, diseases)
        drugbank_ids = [] #None 
        for drug_id, drug_name in drug_to_name.items():
