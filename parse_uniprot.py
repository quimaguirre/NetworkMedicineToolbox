...
def get_uniprot_to_geneid(file_name, uniprot_ids=None, only_min=True, key_function=int):
    ...
    if only_min:
        uniprot_to_geneid = {}
        for uniprot, geneids in uniprot_to_geneids.items():
...
def get_uniprot_to_geneid_from_idmapping_file(file_name, uniprot_ids=None):
    ...
    uniprot_to_geneid = {}
    for uniprot, values in id_to_values.items():
            for val in values:
...
class UniprotXMLElement(object):
    ...
    def __repr__(self):
        return "{}\t{}\t{}\t{}".format(self.ids, self.taxid, self.phosphosites, self.sequence)
